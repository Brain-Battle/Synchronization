...
class GoProRequest:
    ...
    SET_ACCESS_POINT_ON = bytearray([3, 23, 1, 1])
    SET_ACCESS_POINT_OFF = bytearray([3, 23, 1, 0])
    SET_EXTERNAL_CONTROL = bytearray([241, 105, 0])
...
class GoProControl:
    ...
    async def turn_on_ap(self):
        logging.info(f"Turning on AP for: {self._device.address}")
        await self._send_command_request(GoProRequest.SET_ACCESS_POINT_ON)

    async def turn_off_ap(self):
        logging.info(f"Turning off AP for: {self._device.address}")
        await self._send_command_request(GoProRequest.SET_ACCESS_POINT_OFF)
    
    async def set_external_control(self):
        logging.info(f"Setting external control for: {self._device.address}")
        await self._send_command_request(GoProRequest.SET_EXTERNAL_CONTROL)