...
def run_ffmpeg_subprocess(ffmpeg_command: str, resulting_duration: float):
    """
    ...
    try:
        ffmpeg_process = subprocess.Popen(ffmpeg_process, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
    except OSError:
        print("OSError")
    except ValueError:
