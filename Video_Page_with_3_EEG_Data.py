...
class VideoSyncApp(QWidget):
    ...
    def initUI(self):
        ...
        # EEG Plot Panel 3
        self.eeg_data_display_3 = QFrame()
        self.eeg_data_display_3.setFixedSize(800, 150)
        self.eeg_data_display_3.setStyleSheet("border: 2px solid black;")
        self.eeg_plot_layout_3 = QVBoxLayout(self.eeg_data_display_3)
        ...
        self.upload_eeg_btn_3 = QPushButton('Upload EEG 3')
        self.upload_eeg_btn_3.clicked.connect(self.upload_eeg_3)
        ...
        slider_panel.addWidget(self.upload_eeg_btn_3)
        ...
        eeg_data_panel.addWidget(self.eeg_data_display_3, alignment=Qt.AlignCenter)
    ...
    def upload_eeg_3(self):
        options = QFileDialog.Options()
        file_name, _ = QFileDialog.getOpenFileName(
            self, "Open EEG File", "", "CSV Files (*.csv);;All Files (*)", options=options
        )
        if file_name:
            self.process_and_plot_eeg_3(file_name)
    ...
    def process_and_plot_eeg_3(self, file_name):
        CONVERT_TO_DATETIME = False
        PULL_TO_EPOCH = False
        ...
        axs[0].plot(df["timestamps"], df["TP9"], color="brown", linewidth=2)
        axs[1].plot(df["timestamps"], df["AF7"], color="olive", linewidth=2)
        axs[2].plot(df["timestamps"], df["TP10"], color="teal", linewidth=2)
        axs[3].plot(df["timestamps"], df["AF8"], color="magenta", linewidth=2)

        for ax in axs:
            ax.legend(fontsize=5)
            ax.grid(True)

        fig.suptitle("EEG Plot 3", fontsize=8)
        ...
        toolbar = NavigationToolbar(canvas, self)
        toolbar.setFixedHeight(25)
        self.eeg_plot_layout_3.addWidget(toolbar)
        self.eeg_plot_layout_3.addWidget(canvas)
